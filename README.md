...
すべてのAIツール（Gemini CLI、Claude Code、Codex CLI）に以下のシステムフォルダへのアクセス制限が設定されています：

**ブロックされるパス:**
- `/etc/*` - システム設定ファイル
- `/usr/bin/*`, `/usr/sbin/*`, `/bin/*`, `/sbin/*` - システムバイナリ
- `/System/*`, `/Library/*` - macOSシステムファイル
- `/var/db/*`, `/var/root/*` - システムデータベース
- `/boot/*` - ブートローダー（Linux）
- `/root/*` - rootユーザーのホーム
...
AIツールは以下のディレクトリでのみ動作すべきです：
- ユーザーホームディレクトリ: `~/`
- プロジェクトディレクトリ
- 一時ディレクトリ: `/tmp/`（注意が必要）

詳細は [AGENTS.md](file:///Users/takurogoto/Devel/github.com/sopra/dotfiles/AGENTS.md) を参照してください。
